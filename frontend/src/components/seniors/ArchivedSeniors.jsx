      ...
      <div className="mb-6 md:mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
           <div className="flex items-center gap-2 text-orange-600 mb-2">
              <div className="p-1.5 md:p-2 bg-orange-100 rounded-lg border border-orange-200 shadow-sm"><ArchiveRestore size={16} strokeWidth={2} /></div>
      ...
      <div className="bg-white border border-stone-300 shadow-sm min-h-[400px] md:min-h-[500px] rounded-2xl overflow-hidden relative">
        <div className="overflow-x-auto w-full max-w-full">
          <table className="w-full text-left border-collapse min-w-[800px]">
            <thead>
              <tr className="bg-orange-50 text-stone-600 text-[10px] md:text-[11px] uppercase font-medium tracking-widest border-b-2 border-orange-200">
            ...
            <tbody className="text-[13px] md:text-sm">
              {loading ? (
                 <tr>
                   <td colSpan="4" className="py-24 text-center"><Loader2 className="animate-spin text-orange-600 mx-auto" size={32} /></td>
                 </tr>
              ) : seniors.length === 0 ? (
                 <tr>
                ...
                seniors.map((r) => (
                  <tr key={r.id} className="border-b border-stone-200 hover:bg-stone-50 transition-colors">
                    <td className="py-3 md:py-4 px-3 md:px-5 align-middle">
                      <span className="font-medium text-stone-500 line-through tracking-tight uppercase">
                    ...
                    <td className="py-3 md:py-4 px-3 md:px-5 text-right align-middle">
                      <div className="flex items-center justify-end gap-1.5 md:gap-2">
                        ...
                        <button onClick={() => setDeleteModal({ isOpen: true, id: r.id, name: `${r.first_name} ${r.last_name}` })} className="p-2 bg-stone-100 text-red-600 hover:bg-red-700 hover:text-white rounded-lg transition-all shadow-sm border border-stone-200" title="Permanently Delete">
      ...
      {deleteModal.isOpen && createPortal(
        <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white w-full max-w-[420px] rounded-2xl shadow-2xl p-6 md:p-8 animate-in zoom-in-95 duration-200">
            <div className="w-14 h-14 bg-red-100 text-red-600 rounded-full flex items-center justify-center mb-5 border-4 border-red-50">
              <ShieldAlert size={28} />
            </div>
            ...
            <div className="flex gap-3">
              ...
              <button onClick={confirmDelete} className="flex-1 px-4 py-3 bg-red-600 text-white rounded-xl hover:bg-red-700 flex items-center justify-center" disabled={isProcessing}>
